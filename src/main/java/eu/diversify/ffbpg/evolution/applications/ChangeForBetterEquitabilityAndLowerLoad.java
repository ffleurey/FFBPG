 ...
 * @author ffl
 */
public class ChangeForBetterEquitabilityAndLowerLoad extends ApplicationEvolutionOperator {
    ...
    public boolean execute(BPGraph graph, Application a) {
        ...
        for (Platform toAdd : can_be_added) {
            ...
            else if (new_equitability > (base_equitability - 0.001* base_equitability) && toRemove.getLoad() > toAdd.getLoad()) {
                a.removeLinkToPlatform(toRemove);
                a.addLinkToPlatform(toAdd);
                System.out.println(" >>>> !!!!!!!!!! DEBUG: NEW LOAD RULE APPLIED !!!!!!!!!! <<<<");
                return true;
            }
            }
        }
