 ...
 * @author ffl
 */
public class ChangePlatformServicesForPopularEvolutionScenario extends PlatformServicesEvolutionScenario {

    public ChangePlatformServicesForPopularEvolutionScenario() {
        super("Change To Popular Services", null, null);
    }
    
    ChangeForMorePopularService operator = new ChangeForMorePopularService();
    ...
    public void step(BPGraph graph) {
      List<Platform> plats = (ArrayList<Platform>)graph.getPlatforms().clone();
       ...
       for(Platform p : plats) {
        ...
        System.out.println("Step complete. Operator applied on " + plats.size() +" platforms and swaped " + op_count + " services for more popular ones.");
